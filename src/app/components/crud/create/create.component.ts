...
export class CreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const imovel = navigation?.extras?.state?.['imovel'] ?? history.state['imovel'];
  ...
  create() {
    ...
    if (this.isEdit) {
      const navigation = this.router.getCurrentNavigation();
      const imovel = navigation?.extras?.state?.['imovel'] ?? history.state['imovel'];
      this.formData.append('formData', JSON.stringify(Imovel.parsePlainObjectIntoImovel( { id: imovel.id, ...this.createImovel.value, imagens: this.previewUrls.filter(({url}) => url.indexOf("blob") === -1).map(({url}) => url.replace("http://asdfimobiliaria.s3-website-sa-east-1.amazonaws.com/", "")) })));      
      this.httpClient.post('https://rn5pjlgu8k.execute-api.sa-east-1.amazonaws.com/imovel?id=' + imovel.id, this.formData).subscribe(console.log); //https://rn5pjlgu8k.execute-api.sa-east-1.amazonaws.com/imovel
    } else {
      this.formData.append('formData', JSON.stringify(Imovel.parsePlainObjectIntoImovel( { ...this.createImovel.value, imagens: this.previewUrls.filter(({url}) => url.indexOf("blob") === -1).map(({url}) => url) })));
